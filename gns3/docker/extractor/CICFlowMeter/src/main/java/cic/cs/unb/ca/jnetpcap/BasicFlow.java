...
import cic.cs.unb.ca.ifm.Label;
...
public class BasicFlow {
    ...
    private Map<Label, Long> countPerLabel = new HashMap<>();
    

	public BasicFlow(Label label, boolean isBidirectional,BasicPacketInfo packet, byte[] flowSrc, byte[] flowDst, int flowSrcPort, int flowDstPort) {
		super();
		this.initParameters();
		this.isBidirectional = isBidirectional;
		this.firstPacket(label, packet);
		this.src = flowSrc;
		this.dst = flowDst;
	...
	public BasicFlow(Label label, boolean isBidirectional,BasicPacketInfo packet) {
		super();
		this.initParameters();
		this.isBidirectional = isBidirectional;
		this.firstPacket(label, packet);
	...
	public BasicFlow(Label label, BasicPacketInfo packet) {
		super();
		this.initParameters();
		this.isBidirectional = true;		
		firstPacket(label, packet);
	}
	...
	public void firstPacket(Label label, BasicPacketInfo packet){
		addLabel(label);
    ...
    public void addPacket(Label label, BasicPacketInfo packet){
		addLabel(label);
		updateFlowBulk(packet);
		detectUpdateSubflows(packet);
		checkFlags(packet);
    ...
    private void addLabel(Label label) {
		Long previousCount = countPerLabel.get(label);
		countPerLabel.put(label, previousCount == null ? 1L : (previousCount + 1L));
	}
	...
	public String getLabel() {
		Label nonBenignLabelWithHighestCount = null;
		long highestCount = 0;
		for(Map.Entry<Label, Long> entry : countPerLabel.entrySet()) {
			if(!Label.BENIGN.equals(entry.getKey()) && entry.getValue() > highestCount) {
				nonBenignLabelWithHighestCount = entry.getKey();
			}
		}
